...
def sendmessage(name):
   ...
   mssg="Your student "+name+" is inside the campus \nTime: "+dtstring+"\r"
...
def insertstudent(name):
   now = datetime.now()
   mycursor = mydb.cursor()
   sql = "INSERT INTO studentin (Name, Date, Time) VALUES (%s,%s,%s)"
...
def pandaIn(name):
   ...
   if name not in Row_list:
      ...
      new_row=pd.DataFrame({'Name':name,'Time':dtString},index = [0])
